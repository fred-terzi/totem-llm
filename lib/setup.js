...
import { migrateDatabaseFilename } from "./migrateDbName.js";
...
function runCommand(cmd, args, { cwd, env } = {}) {
  return new Promise((resolve, reject) => {
    // On Windows, .cmd files must be invoked through the shell (EINVAL otherwise)
...
async function runMigrations(serverDir, env) {
  ...
  await runCommand(prismaBin, ["migrate", "deploy", "--schema", join(serverDir, "prisma", "schema.prisma")], {
    cwd: serverDir,
    env,
  });
}
...
export async function setup({ packageRoot, force = false }) {
  ...
  // 2b. Database filename migration: pre-existing AnythingLLM installs have
  //     `anythingllm.db` in the storage dir. Rename it to the new Totem LLM
  //     default (`totem-llm.db`) BEFORE migrations run, so Prisma opens the
  //     already-migrated file. No-op for fresh installs and Postgres/MySQL.
  migrateDatabaseFilename(storageDir);

  // 3. Feature flags config (copy default if not already present in storage dir)
  const __filename = fileURLToPath(import.meta.url);
  const __dirname = dirname(__filename);
  const featuresConfigDest = join(storageDir, '.totem.features.json');
  const featuresConfigSrc = join(__dirname, '..', 'config', 'totem.features.json');
  if (!existsSync(featuresConfigDest) && existsSync(featuresConfigSrc)) {
    copyFileSync(featuresConfigSrc, featuresConfigDest);
  } else if (existsSync(featuresConfigDest)) {
    console.log(`  Feature flags config found: ${featuresConfigDest}`);
  }
