...
export function getEnv({ storageDir } = {}) {
  ...
  return {
    ...
    STORAGE_DIR: storage,
    // Default SQLite database file. Pre-existing anythingllm.db files are
    // auto-migrated to this name on startup (see migrateDatabaseFilename()).
    DATABASE_URL: `file:${join(storage, "totem-llm.db")}`,
    NODE_ENV: "production",
  };
}
